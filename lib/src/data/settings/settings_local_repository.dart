...
class SettingsLocalRepository extends SettingsRepository {
  ...
  Future<PopularNewsCriterion> getPopularNewsCriterion() async {
    final criterionValue = await _persistenceRepository.getString(_Constants.selectedPopularNewsCriterionKey);
    return PopularNewsCriterion.values.firstWhere((criterion) => criterion.toString() == criterionValue, orElse: () => PopularNewsCriterion.viewed);
  }

  @override
  Future<AppTheme> getTheme() async {
    final themeValue = await _persistenceRepository.getString(_Constants.selectedThemeKey);
    return AppTheme.values.firstWhere((theme) => theme.toString() == themeValue, orElse: () => AppTheme.light);
  }
